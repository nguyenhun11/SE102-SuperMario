...
void MapBackground::Render()
{
	...
	// Chỉ cần chuyển từ góc Top-Left sang Center để Engine vẽ đúng vị trí gốc
	float center_x = this->x + spr->GetWidth() / 2.0f;
	float center_y = this->y + spr->GetHeight() / 2.0f;

	// Dùng Draw (không phải DrawOnScreen) để nó trượt theo Camera
	spr->DrawOnCamera(center_x, center_y);
}
...
//
//	map_scene_X = top_left_x;
//	map_screen_Y = top_left_y;
//
